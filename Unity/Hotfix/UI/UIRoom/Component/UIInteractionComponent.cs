...
namespace Hotfix
{
    ...
    public class InteractionComponent : Component
    {
        ...
        private async void OnPlay()
        {
            ...
            long playerId = ClientComponent.Instance.LocalPlayer.Id;
            PlayCards_RE playCardsRE = await SessionComponent.Instance.Session.Call<PlayCards_RE>(new PlayCards_RT() { PlayerID = playerId, Cards = currentSelectCards.ToArray() });

            //出牌错误提示
            GamerUIComponent gamerUI = Hotfix.Scene.GetComponent<UIComponent>().Get(UIType.Room).GetComponent<GamerComponent>().LocalGamer.GetComponent<GamerUIComponent>();
        ...
        {
            long playerId = ClientComponent.Instance.LocalPlayer.Id;
            Prompt_RE promptRE = await SessionComponent.Instance.Session.Call<Prompt_RE>(new Prompt_RT() { PlayerID = playerId });
