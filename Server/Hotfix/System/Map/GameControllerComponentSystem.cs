...
{
    [ObjectEvent]
    public class GameControllerComponentEvent : ObjectEvent<GameControllerComponent>, IAwake<RoomConfig>
    {
    ...
    public static class GameControllerComponentSystem
    {
        ...
        public static void CardsOnTable(this GameControllerComponent self, long id)
        {
            ...
            //广播地主消息
            room.Broadcast(new SelectLord() { PlayerID = id, LordCards = deskCardsCache.LordCards.ToArray() });

            //广播地主先手出牌消息
            room.Broadcast(new AuthorityPlayCard() { PlayerID = id, IsFirst = true });
        }
        ...
        public static async void GameOver(this GameControllerComponent self, Dictionary<long, long> gamersSorce)
        {
            ...
            for (int i = 0; i < gamers.Length; i++)
            {
                ...
                if (gamerMoney < self.MinThreshold)
                {
                    ...
                    actorProxy.Send(new GamerMoneyLess() { PlayerID = gamers[i].Id });
                    isKickOut = true;
                }
        ...
        public static void RandomFirstAuthority(this GameControllerComponent self)
        {
            ...
            //广播先手玩家
            room.Broadcast(new SelectAuthority() { PlayerID = firstAuthority });
        }
    }
}
